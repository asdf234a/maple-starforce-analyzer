...
function renderKpis(result) {
  if (!result || !result.items || result.items.length === 0) {
    ...
    const budgetSpan = document.getElementById('kpiBudgetSpan');
    if (budgetSpan) budgetSpan.innerText = '-';
  ...
  const budgetSpan = document.getElementById('kpiBudgetSpan');
  if (budgetSpan) {
    budgetSpan.innerText = `${formatMeso(result.percentiles.p10)} ~ ${formatMeso(result.percentiles.p90)}`;
  }
  ...
  document.getElementById('kpiZeroDestroyProb').innerText = `노파괴(0개) 완료 확률: ${pZero}%`;
  ...
  if (state.items.length === 0) {
    container.innerHTML = `
      <div class="empty-items-state">
        ...
        <p>강화할 장비가 없습니다.<br>위 <strong>[+ 장비 추가]</strong> 버튼을 눌러 대상을 등록하세요.</p>
      </div>
    `;
    return;
  }
...
/**
 * 비용 분포도 차트 렌더링 (Chart.js) - 99.995% 초과 롱테일 꼬리 컷오프 적용
 */
function renderCostChart(result) {
  ...
  const { totalCostPMF, totalCostCDF, binSize } = result;

  // 99.995% 초과 롱테일 꼬리 컷오프 (가독성 최적화)
  let cutoffIdx = totalCostPMF.length;
  for (let i = 0; i < totalCostCDF.length; i++) {
    if (totalCostCDF[i] >= 0.99995) {
      cutoffIdx = i + 1;
      break;
    }
  }
  cutoffIdx = Math.max(10, Math.min(totalCostPMF.length, cutoffIdx));
...
/**
 * 파괴 횟수 분포 차트 렌더링 - 99.995% 초과 롱테일 컷오프
 */
function renderDestroyChart(result) {
  ...
  // 99.995% 누적 도달 시점까지만 슬라이스
  let stats = result.destroyStats;
  let cutIdx = stats.length;
  for (let i = 0; i < stats.length; i++) {
    if (stats[i].cumulative >= 0.99995) {
      cutIdx = i + 1;
      break;
    }
  }
...
function renderTables(result) {
  ...
  const percRows = [
    { label: '상위 1%', meaning: '상위 1% 이내 완성 (극상위 행운)', val: p.p1, tag: 'tag-p10' },
    { label: '상위 5%', meaning: '상위 5% 이내 완성', val: p.p5, tag: 'tag-p10' },
    { label: '상위 10%', meaning: '상위 10% 이내 완성', val: p.p10, tag: 'tag-p10' },
    ...
    { label: '상위 99%', meaning: '99% 확률로 이 예산 내 완성', val: p.p99, tag: 'tag-p90' }
  ];
...
function initEvents() {
  ...
  if (btnCopy) {
    btnCopy.addEventListener('click', () => {
      ...
      report += `• 상위 90% (안전선): ${formatMeso(result.percentiles.p90)}\n`;
      report += `• 기대 소모 장비: ${result.totalExpDestroys.toFixed(2)}개 (노파괴 확률 ${(result.totalDestroyPMF[0] * 100).toFixed(2)}%)\n\n`;
