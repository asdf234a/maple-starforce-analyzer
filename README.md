...
- 각 성수($15 \to 22$)별로 장비 레벨과 노작 가격, 이벤트 상태를 고려하여:
  ...
  - **23성 이상**: `[확정복구(22성으로 복구) / 12성 롤백]` 2자 비교
  - 계산 수식은 [STARFORCE.GG 기대값 계산 로직](https://starforce.gg/blog/ev-calculation-logic)과 동일하며, `npm test`로 레퍼런스 값(`test/blog_reference.json`)과의 일치를 검증합니다.
